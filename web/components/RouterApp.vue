...
import { Activity, Bot, Check, ChevronRight, CircleAlert, Copy, KeyRound, Layers3, LayoutDashboard, LogOut, PanelLeftClose, PanelLeftOpen, Plus, RadioTower, RefreshCw, Search, Sparkles, TerminalSquare, UserRound, Users, WalletCards, ReceiptText, Tags, Settings } from 'lucide-vue-next'
import { api, clearToken, getToken, setToken } from '~/src/api'
import type { Account, ActivityLog, ApiKey, CatalogGroup, CatalogModel, Channel, Group, LedgerEntry, ModelProvider, PaymentMethod, PaymentOrder, PaymentSettings, Pricing, SiteSettings, UsageRecord, User } from '~/src/api'
const { locale, t, setLocale, toggleLocale, initializeLocale } = useI18n()

type View = 'overview' | 'users' | 'groups' | 'keys' | 'channels' | 'providers' | 'logs' | 'account' | 'profile' | 'wallet' | 'usage' | 'usage-overview' | 'ledger' | 'pricing' | 'site-settings' | 'payment-settings' | 'audit'
const props = withDefaults(defineProps<{ activeView?: View }>(), { activeView: 'overview' })
const route = useRoute()
const router = useRouter()
const views: View[] = ['overview', 'users', 'groups', 'keys', 'channels', 'providers', 'logs', 'account', 'profile', 'wallet', 'usage', 'usage-overview', 'ledger', 'pricing', 'site-settings', 'payment-settings', 'audit']
...
const adminExtraNav = [['pricing', 'pricing', Tags, 'pricing.read'], ['site-settings', 'siteSettings', Settings, 'system.manage'], ['payment-settings', 'paymentSettings', WalletCards, 'system.manage']] as const
      ...
      <section class="content" :data-usage-page="route.query.view === 'usage-overview' || route.params.view === 'usage-overview' ? 'overview' : 'logs'">
       ...
       <template v-if="view === 'overview'">
        <section class="setup-workspace">
          ...
          <div class="request-preview"><div class="request-preview-title"><span><TerminalSquare :size="16" />{{ t('firstApiRequest') }}</span><code>curl</code></div><pre><span>curl {{ apiEndpoint }} \</span><span>  -H <i>"Authorization: Bearer sk-xh-..."</i> \</span><span>  -H <i>"Content-Type: application/json"</i> \</span><span>  -d <i>'{"model":"kimi-k3-mini",</i></span><span><i>       "messages":[{"role":"user","content":"你好"}]}'</i></span></pre><div class="request-signals"><span><i></i>{{ t('gatewayOnline') }}</span><button @click="openConsole('account')">{{ t('viewKeys') }} <ChevronRight :size="14" /></button></div></div>
        </section>
