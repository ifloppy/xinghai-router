...
<template>
  ...
  <main v-if="isLanding" class="landing-shell">
    ...
    <section id="quickstart" class="quickstart-section"><div><p class="eyebrow">ONE ENDPOINT</p><h2>{{ t('quickstartTagline') }}</h2><p>{{ t('quickstartDesc') }}</p></div><pre><span class="code-comment">// Using OpenAI SDK</span><span><b>const</b> client = <b>new</b> OpenAI({</span><span>  apiKey: <i>'sk-xh-your-key'</i>,</span><span>  baseURL: <i>'http://localhost:8080/v1'</i></span><span>})</span><span class="code-gap"></span><span><b>await</b> client.chat.completions.create({</span><span>  model: <i>'gpt-4o'</i>,</span><span>  messages: [{ role: <i>'user'</i>, content: <i>'你好'</i> }]</span><span>})</span></pre></section>
  ...
  <main v-else class="app-shell">
    <aside class="sidebar">
       ...
       <template v-if="view === 'overview'">
        <section class="setup-workspace">
          <div class="setup-guide">
             <div class="setup-heading"><div><span class="overview-kicker">{{ t('quickStartHeading') }}</span><h2>{{ account?.name || '' }}{{ t('sendModelRequest') }}</h2><p>{{ t('completeThreeSteps') }}</p></div><span class="setup-progress">{{ setupProgress }} / 3</span></div>
          ...
          <div class="request-preview"><div class="request-preview-title"><span><TerminalSquare :size="16" />{{ t('firstApiRequest') }}</span><code>curl</code></div><pre><span>curl {{ apiEndpoint }} \</span><span>  -H <i>"Authorization: Bearer sk-xh-..."</i> \</span><span>  -H <i>"Content-Type: application/json"</i> \</span><span>  -d <i>'{"model":"gpt-4o-mini",</i></span><span><i>       "messages":[{"role":"user","content":"你好"}]}'</i></span></pre><div class="request-signals"><span><i></i>{{ t('gatewayOnline') }}</span><button @click="openConsole('account')">{{ t('viewKeys') }} <ChevronRight :size="14" /></button></div></div>
        </section>
      ...
      <template v-if="view === 'keys'"><section class="toolbar"><div><h2>{{ t('keys') }}</h2><p>{{ t('showOnceAfterCreation') }}</p></div><button class="button primary" :disabled="!users.length" @click="showKey = true"><Plus :size="16" />{{ t('createKeyButton') }}</button></section><section class="panel table-panel"><table><thead><tr><th>{{ t('keyName') }}</th><th>{{ t('userLabel') }}</th><th>{{ t('keyPrefix') }}</th><th>{{ t('lastUsed') }}</th><th>{{ t('accountStatus') }}</th><th></th></tr></thead><tbody><tr v-for="key in keys" :key="key.id"><td><b>{{ key.name }}</b></td><td>{{ userName(key.user_id) }}</td><td><code>{{ key.key_prefix }}...</code></td><td>{{ formatDate(key.last_used_at) }}</td><td><span :class="['state', key.revoked_at ? 'bad' : 'good']">{{ key.revoked_at ? t('revoked') : t('valid') }}</span></td><td><button v-if="!key.revoked_at" class="text-button danger" @click="revokeKey(key)">{{ t('revokeLabel') }}</button></td></tr></tbody></table><Empty v-if="!keys.length" :text="t('createUserThenIssueKey')" /></section></template>
        <template v-if="view === 'channels'"><section class="toolbar"><div><h2>{{ t('upstreamChannels') }}</h2><p>{{ t('channelsDesc') }}</p></div><button v-if="can('channels.manage')" class="button primary" @click="showChannel = true"><Plus :size="16" />{{ t('addChannel') }}</button></section><section class="panel table-panel channel-table-panel"><table><thead><tr><th>Status</th><th>Channel name</th><th>Upstream URL</th><th>{{ t('modelLabel') }}</th><th>{{ t('priorityLabel') }}</th><th v-if="can('channels.manage')">{{ t('enableChannelLabel') }}</th><th v-if="can('channels.manage')"></th></tr></thead><tbody><tr v-for="channel in channels" :key="channel.id"><td><span :class="['state', channel.enabled ? 'good' : 'bad']"><i :class="['status-dot', { off: !channel.enabled }]"></i>{{ channel.enabled ? t('enabled') : t('disabled') }}</span></td><td><b>{{ channel.name }}</b><small>{{ channel.provider }}</small></td><td><code :title="channel.base_url">{{ channel.base_url }}</code></td><td><div class="model-tags"><span v-for="model in channel.models" :key="model">{{ model }}</span></div></td><td>{{ channel.priority }}</td><td v-if="can('channels.manage')"><button class="toggle" :class="{ on: channel.enabled }" :aria-label="channel.enabled ? t('disableChannelLabel') : t('enableChannelLabel')" @click="toggleChannel(channel)"><i></i></button></td><td v-if="can('channels.manage')"><button class="text-button" @click="editChannel(channel)">{{ t('edit') }}</button></td></tr></tbody></table><Empty v-if="!channels.length" :text="t('addOpenAICompatibleUpstream')" /></section></template>
