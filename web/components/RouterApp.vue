...
import { Activity, Bot, Check, ChevronRight, CircleAlert, Copy, KeyRound, LayoutDashboard, LogOut, Moon, Plus, RadioTower, RefreshCw, Sun, TerminalSquare, UserRound, Users, WalletCards, ReceiptText, Tags } from 'lucide-vue-next'
...
const accountKeyForm = reactive({ name: '', expires_at: '' })
...
<template>
  ...
  <main v-else class="app-shell">
    ...
    <div v-if="selectedUser || showKey || showAccountKey || showChannel || createdKey" class="modal-backdrop" @click.self="selectedUser = null; showKey = showAccountKey = showChannel = false">
      ...
      <form v-if="showAccountKey" class="modal" @submit.prevent="createAccountKey"><div class="modal-title"><h2>创建 API 密钥</h2><button type="button" @click="showAccountKey = false">×</button></div><p class="muted">密钥将归属当前账户，仅在创建后显示一次。</p><label>密钥名称<input v-model="accountKeyForm.name" required maxlength="100" placeholder="例如：本地开发" /></label><label>过期时间 <small>可选</small><input v-model="accountKeyForm.expires_at" type="datetime-local" /></label><button class="button primary full" :disabled="busy">创建密钥</button></form>
