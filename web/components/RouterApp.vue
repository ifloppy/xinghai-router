...
import { Activity, Bot, Check, ChevronRight, CircleAlert, Copy, KeyRound, Layers3, LayoutDashboard, LogOut, Moon, Plus, RadioTower, RefreshCw, Search, Sparkles, Sun, TerminalSquare, UserRound, Users, WalletCards, ReceiptText, Tags, Settings } from 'lucide-vue-next'
import { api, clearToken, getToken, setToken } from '~/src/api'
import type { Account, ActivityLog, ApiKey, CatalogGroup, CatalogModel, Channel, Group, LedgerEntry, ModelProvider, Pricing, SiteSettings, UsageRecord, User } from '~/src/api'
const { locale, t, setLocale, toggleLocale, initializeLocale } = useI18n()

type View = 'overview' | 'users' | 'groups' | 'keys' | 'channels' | 'providers' | 'logs' | 'account' | 'profile' | 'wallet' | 'usage' | 'usage-overview' | 'ledger' | 'pricing' | 'site-settings' | 'audit'
const props = withDefaults(defineProps<{ activeView?: View }>(), { activeView: 'overview' })
const route = useRoute()
const router = useRouter()
const views: View[] = ['overview', 'users', 'groups', 'keys', 'channels', 'providers', 'logs', 'account', 'profile', 'wallet', 'usage', 'usage-overview', 'ledger', 'pricing', 'site-settings', 'audit']
...
const managementNavItems = [
  ['users', 'users', Users, 'users.read'], ['groups', 'groups', Layers3, 'system.manage'], ['keys', 'keys', KeyRound, 'keys.manage'], ['channels', 'channels', RadioTower, 'channels.read'], ['providers', '供应商', Tags, 'system.manage'],
] as const
const adminExtraNav = [['pricing', 'pricing', Tags, 'pricing.read'], ['site-settings', 'siteSettings', Settings, 'system.manage']] as const
...
const siteSettingsForm = reactive<SiteSettings>({ name: '', icon_url: '' })
...
const providerIcon = (slug: string) => `https://unpkg.com/@lobehub/icons-static-svg@latest/light/${slug}.svg`
const modelProvider = (model: string) => catalog.value.find((item) => item.model === model)?.provider ?? '其他'
...
async function syncNewAPIPricing() { await action(async () => { const result = await api<{ synced: number }>('/admin/pricing/newapi/sync', { method: 'POST', body: JSON.stringify(newAPIPricingForm) }); await load(); error.value = `已同步 ${result.synced} 个模型的定价` }) }
...
async function chooseAvatar(event: Event) {
  ...
  if (!file) return
  if (!['image/png', 'image/jpeg', 'image/gif', 'image/webp'].includes(file.type) || file.size > 1.5 * 1024 * 1024) { error.value = '请选择 PNG、JPEG、GIF 或 WebP 图片，大小不超过 1.5 MB'; return }
  await action(async () => {
    const avatarURL = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(new Error('头像读取失败')); reader.readAsDataURL(file) })
...
<template>
  ...
  <main v-if="isLanding" class="landing-shell">
    <nav class="landing-nav">
       ...
       <div class="landing-links"><a href="#features">{{ locale === 'zh-CN' ? '能力' : 'Features' }}</a><a href="/rankings">{{ t('rankings') }}</a><a href="#quickstart">{{ t('quickStart') }}</a><a href="/models">{{ t('marketplace') }}</a><button class="theme-toggle" :aria-label="theme === 'dark' ? t('lightMode') : t('darkMode')" :title="theme === 'dark' ? t('lightMode') : t('darkMode')" @click="toggleTheme"><Sun v-if="theme === 'dark'" :size="16" /><Moon v-else :size="16" /></button><select v-model="locale" class="language-select" :aria-label="t('switchLanguage')"><option value="zh-CN">{{ t('chinese') }}</option><option value="en-US">{{ t('english') }}</option></select><button class="button ghost" @click="openConsoleOrAuth">{{ t('console') }} <ChevronRight :size="15" /></button></div>
    </nav>
    ...
    <section id="quickstart" class="quickstart-section"><div><p class="eyebrow">ONE ENDPOINT</p><h2>接入只需要<br><em>改一个地址。</em></h2><p>使用熟悉的 OpenAI SDK，将 Base URL 指向 Xinghai Router，即刻获得统一的模型入口。</p></div><pre><span class="code-comment">// 使用 OpenAI SDK</span><span><b>const</b> client = <b>new</b> OpenAI({</span><span>  apiKey: <i>'sk-xh-your-key'</i>,</span><span>  baseURL: <i>'http://localhost:8080/v1'</i></span><span>})</span><span class="code-gap"></span><span><b>await</b> client.chat.completions.create({</span><span>  model: <i>'gpt-4o'</i>,</span><span>  messages: [{ role: <i>'user'</i>, content: <i>'你好'</i> }]</span><span>})</span></pre></section>
     <footer class="landing-footer"><span>© 2026 Xinghai Router</span><span>轻量、透明、为模型流量而生。</span><span class="legal-links"><a href="/terms">{{ locale === 'zh-CN' ? '用户协议' : 'Terms' }}</a><a href="/privacy">{{ locale === 'zh-CN' ? '隐私协议' : 'Privacy' }}</a></span></footer>
  </main>

  <main v-else-if="isMarketplacePage" class="public-marketplace">
      <nav class="marketplace-nav"><a class="landing-logo" href="/"><span class="brand-mark small"><Bot :size="19" /></span><span>{{ siteSettings.name }}</span></a><div><button class="theme-toggle" :aria-label="theme === 'dark' ? t('lightMode') : t('darkMode')" @click="toggleTheme"><Sun v-if="theme === 'dark'" :size="16" /><Moon v-else :size="16" /></button><button class="language-toggle" :aria-label="t('switchLanguage')" @click="toggleLocale">{{ locale === 'zh-CN' ? 'EN' : '中' }}</button><button class="button ghost" @click="openConsoleOrAuth">{{ authenticated ? t('console') : t('login') }} <ChevronRight :size="15" /></button></div></nav>
    <section class="marketplace-page-content">
     ...
     <main v-else-if="isAuthPage" class="login-shell">
      <section class="login-card"><div class="login-card-actions"><button class="theme-toggle" :aria-label="theme === 'dark' ? t('lightMode') : t('darkMode')" @click="toggleTheme"><Sun v-if="theme === 'dark'" :size="16" /><Moon v-else :size="16" /></button><button class="language-toggle" :aria-label="t('switchLanguage')" @click="toggleLocale">{{ locale === 'zh-CN' ? 'EN' : '中' }}</button><button class="login-close" aria-label="返回首页" @click="closeAuth">×</button></div><div class="brand-mark"><Bot :size="29" /></div><p class="eyebrow">XINGHAI ROUTER</p><h1>{{ locale === 'zh-CN' ? '控制模型流量。' : 'Control your model traffic.' }}</h1><div class="auth-tabs"><button :class="{ active: loginMode === 'login' }" @click="loginMode = 'login'">{{ locale === 'zh-CN' ? '账户登录' : 'Sign in' }}</button><button :class="{ active: loginMode === 'register' }" @click="loginMode = 'register'">{{ locale === 'zh-CN' ? '创建账户' : 'Create account' }}</button></div><form @submit.prevent="accountSignIn(loginMode === 'register')"><label v-if="loginMode === 'register'">{{ locale === 'zh-CN' ? '姓名' : 'Name' }}<input v-model="accountForm.name" autocomplete="name" required maxlength="100" :placeholder="locale === 'zh-CN' ? '例如：李雷' : 'e.g. Alex'" /></label><label>{{ locale === 'zh-CN' ? '邮箱' : 'Email' }}<input v-model="accountForm.email" type="email" autocomplete="email" required placeholder="name@example.com" /></label><label>{{ locale === 'zh-CN' ? '密码' : 'Password' }}<input v-model="accountForm.password" type="password" :autocomplete="loginMode === 'register' ? 'new-password' : 'current-password'" required minlength="8" :placeholder="locale === 'zh-CN' ? '至少 8 个字符' : 'At least 8 characters'" /></label><button class="button primary full" :disabled="busy">{{ loginMode === 'register' ? (locale === 'zh-CN' ? '创建并进入控制台' : 'Create and open console') : (locale === 'zh-CN' ? '登录控制台' : 'Sign in to console') }} <ChevronRight :size="16" /></button></form><p class="auth-legal">{{ locale === 'zh-CN' ? '继续即表示您同意' : 'By continuing, you agree to the' }} <a href="/terms">{{ locale === 'zh-CN' ? '用户协议' : 'Terms' }}</a> {{ locale === 'zh-CN' ? '和' : 'and' }} <a href="/privacy">{{ locale === 'zh-CN' ? '隐私协议' : 'Privacy Policy' }}</a>。</p></section>
  </main>
  ...
  <main v-else class="app-shell">
    <aside class="sidebar">
      ...
      <section class="content" :data-usage-page="route.query.view === 'usage-overview' || route.params.view === 'usage-overview' ? 'overview' : 'logs'">
        <header class="console-header"><div><p class="eyebrow">{{ managementNav.some((item) => item[0] === view) ? t('management') : personalNav.some((item) => item[0] === view) ? t('personal') : billingNav.some((item) => item[0] === view) ? t('billing') : t('general') }}</p><h1>{{ [...localizedManagementNavItems, ...generalNav, ...billingNav, ...personalNav, ...localizedAdminExtraNav].find((item) => item[0] === view)?.[1] }}</h1></div><div class="header-actions"><a class="button ghost marketplace-link" href="/models"><Sparkles :size="15" />{{ t('marketplace') }}</a><span class="account-chip"><i>{{ account?.name?.slice(0, 1) || '?' }}</i>{{ account?.name || '正在加载' }}</span><button class="theme-toggle" :aria-label="theme === 'dark' ? t('lightMode') : t('darkMode')" :title="theme === 'dark' ? t('lightMode') : t('darkMode')" @click="toggleTheme"><Sun v-if="theme === 'dark'" :size="16" /><Moon v-else :size="16" /></button><select v-model="locale" class="language-select" :aria-label="t('switchLanguage')"><option value="zh-CN">{{ t('chinese') }}</option><option value="en-US">{{ t('english') }}</option></select><button class="button ghost" @click="load" :disabled="busy"><RefreshCw :size="16" :class="{ spinning: busy }" />{{ t('refresh') }}</button></div></header>
      ...
      <template v-if="view === 'channels'"><section class="toolbar"><div><h2>上游渠道</h2><p>模型请求按渠道优先级进行选择。</p></div><button v-if="can('channels.manage')" class="button primary" @click="showChannel = true"><Plus :size="16" />添加渠道</button></section><div class="channel-cards"><article v-for="channel in channels" :key="channel.id" class="panel channel-card"><div class="card-top"><span :class="['status-dot', { off: !channel.enabled }]"></span><span>优先级 {{ channel.priority }}</span><button v-if="can('channels.manage')" class="toggle" :class="{ on: channel.enabled }" :aria-label="channel.enabled ? '停用渠道' : '启用渠道'" @click="toggleChannel(channel)"><i></i></button></div><h3>{{ channel.name }}</h3><p class="url">{{ channel.base_url }}</p><div class="model-tags"><span v-for="model in channel.models" :key="model">{{ model }}</span></div></article><Empty v-if="!channels.length" text="添加 OpenAI-compatible 上游开始路由" /></div></template>
            <template v-if="view === 'account'"><section class="toolbar"><div><h2>API 密钥</h2><p>用于访问 OpenAI-compatible 模型接口。</p></div><button class="button primary" @click="showAccountKey = true"><Plus :size="16" />创建密钥</button></section><section class="panel table-panel"><table><thead><tr><th>名称</th><th>密钥前缀</th><th>创建时间</th><th>最后使用</th><th>状态</th><th></th></tr></thead><tbody><tr v-for="key in accountKeys" :key="key.id"><td><b>{{ key.name }}</b></td><td><code>{{ key.key_prefix }}...</code></td><td>{{ formatDate(key.created_at) }}</td><td>{{ formatDate(key.last_used_at) }}</td><td><span :class="['state', key.revoked_at ? 'bad' : 'good']">{{ key.revoked_at ? '已吊销' : '有效' }}</span></td><td><button v-if="!key.revoked_at" class="text-button" @click="editAccountKey(key)">编辑</button></td></tr></tbody></table><Empty v-if="!accountKeys.length" text="尚未创建 API 密钥" /></section></template>
           <template v-if="view === 'profile'"><section class="profile-layout"><section class="panel account-card"><div class="profile-avatar">{{ account?.name?.slice(0, 1) || '?' }}</div><div><span class="overview-kicker">账户资料</span><h2>{{ account?.name }}</h2><p>{{ account?.email }}</p></div></section><section class="panel profile-details"><div><span>账户角色</span><b>{{ account?.role }}</b></div><div><span>用户分组</span><div v-if="ownGroups.length" class="profile-group-tags"><span v-for="group in ownGroups" :key="group">{{ group }}</span></div><b v-else>未加入分组</b></div><div><span>账户 ID</span><code>{{ account?.id }}</code></div><div><span>权限范围</span><b>{{ account?.role === 'admin' ? '全部权限' : account?.permissions.join(', ') || '普通账户' }}</b></div></section></section></template>
        <template v-if="view === 'wallet'"><section class="wallet-hero"><div><span>可用余额</span><strong>{{ Number(account?.balance ?? 0).toFixed(4) }}</strong><p>余额可用于后续模型调用费用结算。</p></div><WalletCards :size="64" /></section><div class="metrics wallet-metrics"><article><span>当前余额</span><strong>{{ Number(account?.balance ?? 0).toFixed(4) }}</strong><p><WalletCards :size="15" />账户可用额度</p></article><article><span>预扣金额</span><strong>{{ Number(account?.reserved ?? 0).toFixed(4) }}</strong><p>并发请求中的预留费用</p></article><article><span>累计消费</span><strong>{{ personalCost.toFixed(6) }}</strong><p><ReceiptText :size="15" />最近 100 条用量</p></article></div><section class="panel table-panel"><div class="panel-title"><div><h2>余额流水</h2><p>充值、扣费及退款记录</p></div><button class="text-button" @click="openConsole('ledger')">查看全部</button></div><table><thead><tr><th>时间</th><th>类型</th><th>变动</th><th>余额</th><th>说明</th></tr></thead><tbody><tr v-for="item in ledger.slice(0, 10)" :key="item.id"><td>{{ formatDate(item.created_at) }}</td><td>{{ item.kind }}</td><td :class="item.amount < 0 ? 'danger' : 'success'">{{ item.amount }}</td><td>{{ item.balance_after }}</td><td>{{ item.note || item.request_id }}</td></tr></tbody></table><Empty v-if="!ledger.length" text="暂无余额流水" /></section></template>
         ...
         <template v-if="view === 'site-settings'"><section class="toolbar"><div><h2>{{ t('siteSettings') }}</h2><p>配置站点名称和浏览器标签图标。</p></div></section><form class="panel pricing-form" @submit.prevent="saveSiteSettings"><label>站点名称<input v-model="siteSettingsForm.name" required maxlength="100" /></label><label>站点图标 URL<input v-model="siteSettingsForm.icon_url" type="url" placeholder="https://example.com/favicon.png" /></label><button class="button primary" :disabled="busy">保存设置</button></form></template>
        <template v-if="view === 'pricing'"><section class="toolbar"><div><h2>模型定价</h2><p>按百万 token 配置输入、缓存输入和输出价格。</p></div></section><form v-if="can('pricing.manage')" class="panel pricing-form" @submit.prevent="syncNewAPIPricing"><label>NewAPI 地址<input v-model="newAPIPricingForm.base_url" required type="url" placeholder="https://newapi.example.com" /></label><label>登录令牌<input v-model="newAPIPricingForm.api_key" type="password" placeholder="可选" /></label><label>每 QuotaPerUnit 配额价格<input v-model.number="newAPIPricingForm.price_per_quota_unit" required type="number" min="0" step="any" /><small>NewAPI 的 quota_per_unit 为 500000 时，500000 配额的价格填 1 表示 1 个本地货币单位。</small></label><button class="button primary" :disabled="busy">从 NewAPI 同步</button></form><form v-if="can('pricing.manage')" class="panel pricing-form" @submit.prevent="savePricing"><label>模型<input v-model="pricingForm.model" required placeholder="例如 gpt-4o" /></label><label>输入价格<input v-model.number="pricingForm.input_per_million" type="number" min="0" step="any" placeholder="0" /></label><label>缓存输入<input v-model.number="pricingForm.cached_input_per_million" type="number" min="0" step="any" placeholder="0" /></label><label>输出价格<input v-model.number="pricingForm.output_per_million" type="number" min="0" step="any" placeholder="0" /></label><label>倍率<input v-model.number="pricingForm.multiplier" type="number" min="0.01" step="any" placeholder="1" /></label><button class="button primary">保存规则</button></form><section class="panel table-panel"><table><thead><tr><th>模型</th><th>输入</th><th>缓存输入</th><th>输出</th><th>倍率</th></tr></thead><tbody><tr v-for="item in pricing" :key="item.id"><td><code>{{ item.model }}</code></td><td>{{ item.input_per_million }}</td><td>{{ item.cached_input_per_million }}</td><td>{{ item.output_per_million }}</td><td>{{ item.multiplier }}</td></tr></tbody></table><Empty v-if="!pricing.length" text="暂无模型定价规则" /></section></template>
     </section>
     <section v-if="view === 'profile'" class="panel avatar-settings"><div><h2>头像</h2><p>支持 PNG、JPEG、GIF 和 WebP，最大 1.5 MB。</p></div><img v-if="account?.avatar_url" class="profile-avatar" :src="account.avatar_url" alt="用户头像" /><span v-else class="profile-avatar">{{ account?.name?.slice(0, 1) || '?' }}</span><input ref="avatarInput" class="visually-hidden" type="file" accept="image/png,image/jpeg,image/gif,image/webp" @change="chooseAvatar" /><div><button class="button ghost" type="button" @click="avatarInput?.click()">{{ account?.avatar_url ? '更换头像' : '设置头像' }}</button><button v-if="account?.avatar_url" class="text-button danger" type="button" @click="removeAvatar">移除</button></div></section>

       <div v-if="selectedUser || showKey || showAccountKey || editingAccountKey || showChannel || showProvider || createdKey" class="modal-backdrop" @click.self="selectedUser = null; editingAccountKey = null; showKey = showAccountKey = showChannel = showProvider = false">
      ...
      <form v-if="showKey" class="modal" @submit.prevent="createKey"><div class="modal-title"><h2>创建 API 密钥</h2><button type="button" @click="showKey = false">×</button></div><label>用户<select v-model="keyForm.user_id" required><option disabled value="">选择用户</option><option v-for="user in users" :key="user.id" :value="user.id">{{ user.name }} · {{ user.email }}</option></select></label><label>使用分组<select v-model="keyForm.group_id"><option value="">自动匹配（1.00x）</option><option v-for="group in groups.filter((item) => users.find((user) => user.id === keyForm.user_id)?.groups.includes(item.id))" :key="group.id" :value="group.id">{{ group.name }} · {{ Number(group.multiplier).toFixed(2) }}x</option></select></label><label>密钥名称<input v-model="keyForm.name" required placeholder="例如：生产环境" /></label><label>过期时间 <small>可选</small><input v-model="keyForm.expires_at" type="datetime-local" /></label><button class="button primary full" :disabled="busy">签发密钥</button></form>
       <form v-if="showAccountKey || editingAccountKey" class="modal" @submit.prevent="editingAccountKey ? updateAccountKey() : createAccountKey()"><div class="modal-title"><h2>{{ editingAccountKey ? '编辑 API 密钥' : '创建 API 密钥' }}</h2><button type="button" @click="showAccountKey = false; editingAccountKey = null">×</button></div><p v-if="!editingAccountKey" class="muted">密钥将归属当前账户，仅在创建后显示一次。</p><label>使用分组<select v-model="accountKeyForm.group_id"><option value="">自动匹配（1.00x）</option><option v-for="group in groups.filter((item) => ownGroups.includes(item.name))" :key="group.id" :value="group.id">{{ group.name }} · {{ Number(group.multiplier).toFixed(2) }}x</option></select></label><label>密钥名称<input v-model="accountKeyForm.name" required maxlength="100" placeholder="例如：本地开发" /></label><label>过期时间 <small>可选</small><input v-model="accountKeyForm.expires_at" type="datetime-local" /></label><button class="button primary full" :disabled="busy">{{ editingAccountKey ? '保存修改' : '创建密钥' }}</button></form>
       <form v-if="showProvider" class="modal" @submit.prevent="saveProvider"><div class="modal-title"><h2>配置模型供应商</h2><button type="button" @click="showProvider = false">×</button></div><p class="muted">按模型名称前缀匹配供应商，多个前缀用逗号分隔。</p><label>供应商名称<input v-model="providerForm.name" required placeholder="例如：OpenAI" /></label><label>图标标识<input v-model="providerForm.slug" required placeholder="例如：openai" /></label><label>模型名前缀<input v-model="providerForm.prefixes" required placeholder="例如：gpt-, o1, o3" /></label><label>匹配优先级<input v-model.number="providerForm.priority" required type="number" min="0" /></label><button class="button primary full" :disabled="busy">保存供应商</button></form>
