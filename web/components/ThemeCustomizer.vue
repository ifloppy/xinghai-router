  ...
  <div ref="panel" class="theme-customizer">
    ...
    <Transition name="theme-panel">
      <section v-if="open" class="theme-panel" role="dialog" :aria-label="locale === 'zh-CN' ? '主题配置' : 'Theme settings'">
        ...
        <div class="theme-section"><span>{{ locale === 'zh-CN' ? '预设' : 'Preset' }}</span><button class="theme-preset-card" :class="{ active: preset === 'a-site' }" @click="setPreset('a-site')"><i></i><span><b>Claude 官网</b><small>{{ locale === 'zh-CN' ? 'Anthropic 奶油白与 Claude 橙' : 'Anthropic cream with Claude orange' }}</small></span><Check v-if="preset === 'a-site'" :size="15" /></button></div>
        ...
        <div class="theme-section"><span>{{ locale === 'zh-CN' ? '主色' : 'Color' }}</span><div class="theme-color-grid"><button v-for="item in colors" :key="item.value" :class="['theme-color', `theme-color-${item.value}`, { active: color === item.value }]" @click="setColor(item.value)"><i></i>{{ label(item) }}<Check v-if="color === item.value" :size="14" /></button></div></div>
