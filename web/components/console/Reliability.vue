...
<template>
  ...
  <form class="panel pricing-form reliability-form" @submit.prevent="saveReliabilitySettings">
    ...
    <label>{{ t('healthCheckChannelIds') }}<textarea v-model="reliabilityForm.health_check_channel_ids" rows="3" :placeholder="t('healthCheckChannelIdsPlaceholder')"/><small>{{ t('healthCheckChannelIdsHint') }}</small></label>
    ...
    <label>{{ t('autoDisableKeywords') }}<textarea v-model="reliabilityForm.auto_disable_keywords" rows="8" :placeholder="t('autoDisableKeywordsPlaceholder')"/><small>{{ t('autoDisableKeywordsHint') }}</small></label>
