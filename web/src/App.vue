...
const adminExtraNav = [['pricing', '模型定价', Tags], ['audit', '操作审计', ReceiptText]] as const
...
async function load() {
  ...
  try {
    if (mode.value === 'user') { const [me, ownKeys, usage, wallet] = await Promise.all([api<Account>('/me'), api<{ data: ApiKey[] }>('/me/keys'), api<{ data: UsageRecord[] }>('/me/usage'), api<{ data: LedgerEntry[] }>('/me/ledger')]); account.value = me; keys.value = ownKeys.data; usageRecords.value = usage.data; ledger.value = wallet.data; return }
    const [userResponse, keyResponse, channelResponse, logResponse, pricingResponse, auditResponse] = await Promise.all([
      api<{ data: User[] }>('/admin/users'), api<{ data: ApiKey[] }>('/admin/keys'), api<{ data: Channel[] }>('/admin/channels'), api<{ data: RequestLog[] }>('/admin/request-logs'), api<{ data: Pricing[] }>('/admin/pricing'), api<{ data: Record<string, unknown>[] }>('/admin/audit-logs'),
    ])
    users.value = userResponse.data; keys.value = keyResponse.data; channels.value = channelResponse.data; logs.value = logResponse.data; pricing.value = pricingResponse.data; auditLogs.value = auditResponse.data
  } catch (cause) { error.value = cause instanceof Error ? cause.message : '加载失败' } finally { busy.value = false }
}
async function signIn() { setToken(adminToken.value); error.value = ''; try { await api<Account>('/me'); mode.value = 'user'; view.value = 'account' } catch { mode.value = 'admin'; view.value = 'overview' } authenticated.value = true; await load(); if (error.value) { clearToken(); authenticated.value = false } }
...
<template>
  <main v-if="!authenticated" class="landing-shell">
    <nav class="landing-nav">
      <a class="landing-logo" href="/"><span class="brand-mark small"><Bot :size="19" /></span><span>Xinghai</span><i>Router</i></a>
      <div class="landing-links"><a href="#features">能力</a><a href="#quickstart">快速开始</a><button class="button ghost" @click="showLogin = true">进入控制台 <ChevronRight :size="15" /></button></div>
    </nav>
    ...
    <div v-if="showLogin" class="modal-backdrop" @click.self="showLogin = false"><section class="login-card"><button class="login-close" @click="showLogin = false">×</button><div class="brand-mark"><Bot :size="29" /></div><p class="eyebrow">XINGHAI ROUTER</p><h1>控制模型流量。</h1><p class="muted">使用管理员令牌或 API Key 登录控制台。</p><form @submit.prevent="signIn"><label>访问令牌<input v-model="adminToken" type="password" autocomplete="current-password" placeholder="ADMIN_TOKEN 或 sk-xh-..." required /></label><button class="button primary full" :disabled="busy">进入控制台 <ChevronRight :size="16" /></button></form><p v-if="error" class="error"><CircleAlert :size="16" />{{ error }}</p></section></div>
  </main>

  <main v-else class="app-shell">
    <aside class="sidebar">
       <div class="logo"><span class="brand-mark small"><Bot :size="19" /></span><span>Xinghai</span><i>Router</i></div>
       <nav><template v-if="mode === 'admin'"><button v-for="[id, label, Icon] in nav" :key="id" :class="{ active: view === id }" @click="view = id"><component :is="Icon" :size="18" />{{ label }}</button><button v-for="[id, label, Icon] in adminExtraNav" :key="id" :class="{ active: view === id }" @click="view = id"><component :is="Icon" :size="18" />{{ label }}</button></template><template v-else><button v-for="[id, label, Icon] in userNav" :key="id" :class="{ active: view === id }" @click="view = id"><component :is="Icon" :size="18" />{{ label }}</button></template></nav>
    ...
    <section class="content">
      ...
      <template v-if="view === 'channels'"><section class="toolbar"><div><h2>上游渠道</h2><p>模型请求按渠道优先级进行选择。</p></div><button class="button primary" @click="showChannel = true"><Plus :size="16" />添加渠道</button></section><div class="channel-cards"><article v-for="channel in channels" :key="channel.id" class="panel channel-card"><div class="card-top"><span :class="['status-dot', { off: !channel.enabled }]"></span><span>优先级 {{ channel.priority }}</span><button class="toggle" :class="{ on: channel.enabled }" @click="toggleChannel(channel)"><i></i></button></div><h3>{{ channel.name }}</h3><p class="url">{{ channel.base_url }}</p><div class="model-tags"><span v-for="model in channel.models" :key="model">{{ model }}</span></div></article><Empty v-if="!channels.length" text="添加 OpenAI-compatible 上游开始路由" /></div></template>
       ...
       <template v-if="view === 'account'"><div class="metrics"><article><span>当前余额</span><strong>{{ Number(account?.balance ?? 0).toFixed(4) }}</strong><p><WalletCards :size="15" />可用余额</p></article><article><span>预扣金额</span><strong>{{ Number(account?.reserved ?? 0).toFixed(4) }}</strong><p>并发请求保护</p></article></div><section class="panel account-card"><h2>{{ account?.name }}</h2><p>{{ account?.email }} · {{ account?.role }}</p><code>{{ account?.key_id }}</code></section><section class="panel table-panel account-keys"><div class="panel-title"><div><h2>我的 API Key</h2><p>仅显示前缀，完整密钥不会再次显示。</p></div></div><table><thead><tr><th>名称</th><th>前缀</th><th>最后使用</th><th>状态</th></tr></thead><tbody><tr v-for="key in keys" :key="key.id"><td>{{ key.name }}</td><td><code>{{ key.key_prefix }}...</code></td><td>{{ formatDate(key.last_used_at) }}</td><td><span :class="['state', key.revoked_at ? 'bad' : 'good']">{{ key.revoked_at ? '已吊销' : '有效' }}</span></td></tr></tbody></table></section></template>
